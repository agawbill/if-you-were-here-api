export class PersonsServices {
  ...
  async findPersonById(personId) {
    try {
      const person = await this.personsModel
        .findById(personId)
        .populate({ path: "createdBy", model: "User", select: "email" })
        .populate({ path: "modifiedBy", model: "User", select: "email" });
      return person;
  ...
  async findPersonByPosition(position) {
    try {
      const person = await this.personsModel.findOne({ position });
      return person;
    } catch (err) {
      throw new Exception(err);
    }
  ...
  async addPerson(person) {
    try {
      ...
      const newPerson = await this.personsModel.create({ ...person, position });
      return newPerson;
    } catch (err) {
      console.log(err);
      throw new Exception(err);
  ...
  async updatePerson(personId, body) {
    try {
      const updatedPerson = await this.personsModel
        .findOneAndUpdate({ _id: personId }, body, { new: true })
        .populate({ path: "createdBy", model: "User", select: "email" })
        .populate({ path: "modifiedBy", model: "User", select: "email" });
