export class ResourcesServices {
  ...
  async findResource(resourceId) {
    try {
      const resource = await this.resourcesModel
        .findById(resourceId)
        .populate({ path: "createdBy", model: "User", select: "email" })
        .populate({ path: "modifiedBy", model: "User", select: "email" });
      return resource;
  ...
  async updateResource(body) {
    try {
      const { id } = body;
      const updatedResource = await this.resourcesModel
  ...
  async patchResource(resourceId, body) {
    try {
      const updatedResource = await this.resourcesModel
        .findOneAndUpdate({ _id: resourceId }, body, { new: true })
        .populate({ path: "createdBy", model: "User", select: "email" })
        .populate({ path: "modifiedBy", model: "User", select: "email" });
      return updatedResource;
    } catch (err) {
  ...
  resourceValidations(body) {
    ...
    if (!url.match(regex)) {
      return { success: false, message: "A valid URL must be used." };
    }
